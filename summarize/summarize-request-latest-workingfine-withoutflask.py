...
# Ngrok URL (Hardcoded)
NGROK_URL = "http://d660-34-16-174-84.ngrok-free.app"  # Replace with your actual Ngrok URL
...
def send_summarization_request(text: str, prompt: str, min_length: int, max_length: int):
    ...
    try:
        response = requests.post(url, data=json.dumps(payload), headers=headers, timeout=400, verify=False)
...
try:
    ...
    # Step 5: Get the prompt for summarization
    prompt = input("Please enter the prompt for summarization or press Enter to use default prompt:\n")
    default_prompt = "لطفا این متن را به صورت خلاصه و به زبان فارسی بنویسید:"
    prompt_to_use = prompt if prompt.strip() else default_prompt
    ...
    min_tokens = 10 * num_paragraphs
    max_tokens = 50 * num_paragraphs

    # Step 8: Send request to the API to get the summary
    print("\nSending request to the API for summarization...")
    summary = send_summarization_request(text_to_summarize, prompt_to_use, min_length=min_tokens, max_length=max_tokens)
