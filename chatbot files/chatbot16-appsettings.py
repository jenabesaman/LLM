...
parser.add_argument('--port', type=int, help='Port to run the server on', default=8503)
args = parser.parse_args()

# Now use args.port instead of hardcoded 8503
...
ElasticUrl = Config.get("elasticsearch", "url")
elastic_ca_certs = Config.get("elasticsearch", "ca_certs")
elastic_verify_certs = Config.get("elasticsearch", "verify_certs")
elastic_index = Config.get("elasticsearch", "index")

EmbedderModel = Config.get("models", "embedder_model")
local_model_path = Config.get("models", "local_model_path")

api_token = Config.get("api", "token")
api_url = Config.get("api", "document_url")
...
generation_kwargs = Config.get("llm", "generation_kwargs")
...
def delete_chat_history_index(session_id=None):
    """
    ...
    try:
        if session_id:
            ...
            url = f"{ElasticUrl}{index_name}"
            response = requests.delete(url, auth=ElasticAuth, headers=headers, verify=elastic_verify_certs)
        else:
            # First get all indices matching the pattern
            url = f"{ElasticUrl}_cat/indices/chat_history_*?format=json"
            response = requests.get(url, auth=ElasticAuth, headers=headers, verify=elastic_verify_certs)
            indices = response.json()

            # Delete each index individually
            for index in indices:
                index_name = index['index']
                delete_url = f"{ElasticUrl}{index_name}"
                delete_response = requests.delete(delete_url, auth=ElasticAuth, headers=headers, verify=elastic_verify_certs)
...
def fetch_chat_history(session_id):
    """
    Fetch the latest question and answer from Elasticsearch using session_id as index.
    """
    index_name = f"chat_history_{session_id}"
    url = f"{ElasticUrl}{index_name}/_doc/history"
    headers = {
        "Content-Type": "application/json; charset=utf-8",
        "Accept": "application/json; charset=utf-8"
    }
    ...
    try:
        response = requests.get(url, auth=ElasticAuth, verify=elastic_verify_certs)

        # Check if the index exists
        if response.status_code == 404:
            logging.info(f"Chat history not found for session_id: {session_id}")
...
def fetch_data_from_api():
    try:
        ...
        response = requests.get(api_url, params=params, verify=False, timeout=2)
...
def fetch_documents_from_elasticsearch(query):
    ...
    """
    url = f"{ElasticUrl}{elastic_index}/_search"
    headers = {
        "Content-Type": "application/json; charset=utf-8",
        "Accept": "application/json; charset=utf-8"
    }
    ...
    try:
        response = requests.post(url, auth=ElasticAuth, headers=headers, json=search_body, verify=elastic_verify_certs)
        response.raise_for_status()
        data = response.json()
...
def save_chat_history(session_id, user_message, assistant_response):
    """
    Save chat history using session_id as index name.
    """
    index_name = f"chat_history_{session_id}"
    url = f"{ElasticUrl}{index_name}/_doc/history"
    headers = {
    ...
    existing_interaction = fetch_chat_history(session_id)
    ...
    try:
        response = requests.put(url, auth=ElasticAuth, headers=headers, json=updated_document, verify=elastic_verify_certs)
        response.raise_for_status()
    except requests.exceptions.RequestException as e:
...
def process_question(question, session_id):
    try:
        ...
        last_interaction = fetch_chat_history(session_id)
        ...
        response_text = res['llm']['replies'][0].text.strip()
        answer = f"You as ai last answer:\n{extract_answer(response_text)}"
        user_message = f"User last message:\n{question}"
        save_chat_history(session_id=session_id, user_message=user_message, assistant_response=answer)
...
def clear_chat_history_index(session_id):
    """
    Clears chat history for specific user.
    """
    try:
        index_name = f"chat_history_{session_id}"
        url = f"{ElasticUrl}{index_name}/_delete_by_query"
        headers = {
            "Content-Type": "application/json; charset=utf-8",
            "Accept": "application/json; charset=utf-8"
        }
        ...
        response = requests.post(url, auth=ElasticAuth, headers=headers, json=delete_query, verify=elastic_verify_certs)
        response.raise_for_status()

        if response.status_code == 200:
            logging.info(f"Chat history cleared for user {session_id}")
        else:
            logging.warning(f"Failed to clear chat history for user {session_id}")
...
def chat():
    try:
        ...
        if question.lower() in ["delete all"]:
            delete_chat_history_index(session_id=None)
            return jsonify({"response": "All chat indexes deleted."}), 200

        if question.lower() in ["clear"]:
            clear_chat_history_index(session_id=session_id)
            return jsonify({"response": f"Chat history of id: {session_id} cleared."}), 200

        if question.lower() in ["delete"]:
            delete_chat_history_index(session_id=session_id)
            return jsonify({"response": f"Chat index of id: {session_id} cleared."}), 200
...
if __name__ == "__main__":
    if initialize_model():
        ...
        app.run(debug=server_debug, host=server_host, port=args.port, use_reloader=False)
    else:
