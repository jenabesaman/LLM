...
def process_request():
    while True:
        try:
            ...
            if request is None:  # Poison pill to stop the thread
                break

            user_id, question = request

            # Process the request
            response = process_question(question, user_id)
            answer = extract_answer(response)
            print(f"Response for user {user_id}: {answer}")
...
def main():
    # Start worker thread
    worker_thread = threading.Thread(target=process_request, daemon=True)
    worker_thread.start()
    ...
    while True:
        try:
            ...
            request_queue.put((user_id, question))
...
if __name__ == "__main__":
    # Initialize the model before starting the chat session
    if initialize_model():
